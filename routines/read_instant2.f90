...
LOGICAL, INTENT(IN), OPTIONAL:: S_FLAG, pcon_flag, flag_endless
CHARACTER(len=200)::fname, source
...
IF (flag_endless) THEN
    IF (fname=='vel') THEN
        read(12) u_tot(:,:,1:nz_tot), v_tot(:,:,1:nz_tot), w_tot(:,:,1:nz_tot),&
            RHSx_tot(:, :, 1:nz_tot), RHSy_tot(:, :, 1:nz_tot), RHSz_tot(:, :, 1:nz_tot), Cs_opt2_tot,&
            F_LM_tot, F_MM_tot, F_QN_tot, F_NN_tot
    ELSE IF (fname=='temp') THEN
        read(12) theta_tot(:,:,1:nz_tot), RHS_T_tot(:,:,1:nz_tot), sgs_t3(:,:), psi_m
    ELSE IF (fname=='con') THEN
        READ(12)  PCon_tot(:,:,1:nz_tot,:), RHS_PCon_tot(:,:,1:nz_tot,:)
    ENDIF
